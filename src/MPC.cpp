...
// The cost is stored is the first element of `fg`.
    ...
    for (int t = 0; t < N - 1; t++) {
      fg[0] += kd * CppAD::pow(vars[delta_start + t], 2);
      fg[0] += ka * CppAD::pow(vars[a_start + t], 2);
    }

    // Minimize the value gap between sequential actuations.
    for (int t = 0; t < N - 2; t++) {
      fg[0] += kds * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += kas * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
...
Results MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  bool ok = true;
  size_t i;
  ...
  for (i = delta_start; i < delta_start + latency_dt; i++) {
  ...
  for (i = a_start; i < a_start + latency_dt; i++) {
